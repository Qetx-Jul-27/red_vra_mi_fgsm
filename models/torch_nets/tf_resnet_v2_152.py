...
class KitModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # 当batch_size=1时 仅输出1维度 改为默认2维
        if len(MMdnn_Output_input.shape) == 1:
            MMdnn_Output_input = MMdnn_Output_input.unsqueeze(0)

        # 输出1000类 不要空白类
        device = torch.device("cuda" if MMdnn_Output_input.is_cuda else "cpu")
        my_output = torch.zeros(MMdnn_Output_input.shape[0], MMdnn_Output_input.shape[1] - 1).to(device)
        for i in range(MMdnn_Output_input.shape[0]):
            my_output[i] = MMdnn_Output_input[i][1:]
