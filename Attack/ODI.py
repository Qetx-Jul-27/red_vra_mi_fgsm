...
def advanced_fgsm(attack_type, model, x, y, target_label=None, num_iter=10, max_epsilon=16, mu=1.0,
    ...
    if 'A' in attack_type:  # ATTA
        ...
        for i in range(x.size()[0]):
            ...
            for ko in range(K_outer):
                ...
                atta_model.train()
                L_T = ce(model(atta_model(temp_x_adv)), y) + alpha_1 * ce(model(atta_model(x)), y) + alpha_2 * mse(
                    atta_model(temp_x_adv),
                    temp_x_adv)  # alpha_2*torch.norm(atta_model(temp_x_adv)-temp_x_adv,p=2)**2 #
